...
public class EquipmentRepository : IEquipmentRepository
{
    ...
    public async Task<Equipment> CreateAsync(EquipmentCreateRequest request, string photoUrl, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var categoryId = await GetOrInsertEquipmentCategoryAsync(conn, transaction, request.CategoryDescription.Trim(), now, cancellationToken);
            var typeId = await GetOrInsertEquipmentTypeAsync(conn, transaction, request.TypeDescription.Trim(), now, cancellationToken);
    ...
    private static async Task<long> GetOrInsertEquipmentCategoryAsync(MySqlConnection conn, MySqlTransaction transaction, string description, DateTime now, CancellationToken cancellationToken)
    {
        const string sqlInsert = @"
        INSERT INTO equipment_category (description, created_date, updated_date)
        VALUES (@Description, @CreatedDate, @UpdatedDate);
        SELECT LAST_INSERT_ID();";
        var id = await conn.ExecuteScalarAsync<ulong>(new CommandDefinition(sqlInsert, new { Description = description, CreatedDate = now, UpdatedDate = now }, transaction: transaction, cancellationToken: cancellationToken));
        return (long)id;
    }

    private static async Task<long> GetOrInsertEquipmentTypeAsync(MySqlConnection conn, MySqlTransaction transaction, string description, DateTime now, CancellationToken cancellationToken)
    {
        const string sqlInsert = @"
        INSERT INTO equipment_type (description, created_date, updated_date)
        VALUES (@Description, @CreatedDate, @UpdatedDate);
        SELECT LAST_INSERT_ID();";
        var id = await conn.ExecuteScalarAsync<ulong>(new CommandDefinition(sqlInsert, new { Description = description, CreatedDate = now, UpdatedDate = now }, transaction: transaction, cancellationToken: cancellationToken));
        return (long)id;
    ...
    public async Task<Equipment?> UpdateAsync(EquipmentUpdateRequest request, CancellationToken cancellationToken = default)
    {
        await using var conn = new MySqlConnection(_connectionString);
        await conn.OpenAsync(cancellationToken);
        try
